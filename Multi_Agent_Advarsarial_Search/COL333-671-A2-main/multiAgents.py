...
def betterEvaluationFunction(currentGameState):
    ...
    DESCRIPTION: 
    There are partia scores for each domain of the state, i.e. 
    1. There are some contribution from ghosts's closeness to pacman
    2. There are some contribution from ghosts being very close to pacman
    2. There are some contribution from food distances from pacman
    4. There are some contribution from food pellets/capsules

    The score if very high if the sate is winning state
    The score is very low if the state is losing state
    There score increses is a ghost is scared and is within mahnatten distance of 8 (given more weight to this for eating ghosts so squared the term)
    There score decreses is a ghost is not scared and is within mahnatten distance of 7 (given more weight to this for eating ghosts so squared the term)
    The ghosts which are far away have no effect on the scores.
    The score decreases to -inf (-1e6) if any ghost is at a mahnatten distance of 1 or less
    The score increases if the food is closer to the pacman (inversey proportional)
    The score decreses if the food is farther from the pacman (inversey proportional)
    The score increases if the closest pellet is closer (inversey proportional)
    The score decreases if the closest pellet is farther (inversey proportional)
    If the no of pellet decreases then score increases(motivates the pacman to eat pellets)
    If the no of food decreases then score increases(motivates the pacman to eat foods)
    The overall score is a weight linear combination of all the partial scores
    """
    "*** YOUR CODE HERE ***"
